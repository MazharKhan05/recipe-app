...
export default function Login() {
  ...
  const handleLogin = async (e)=>{
    ...
    const loginRes = await fetch(`${process.env.STRAPI_API_URL}/api/auth/local`, {
      method: "POST",
      headers:{
  ...
  return (
    <>
      <Header />
      <ToastContainer />
      <div className="bg-red-400 h-screen w-screen flex items-center justify-center py-12 px-4 sm:px-6 lg:px-8 ">
        <div className="w-full max-w-md space-y-8">
          ...
          <a className={`${!user ? 'inline' : 'hidden'}`} href={`${process.env.STRAPI_API_URL}/api/connect/cognito`}>
          ...
          <a className={`${!user ? 'inline' : 'hidden'}`} href={`${process.env.STRAPI_API_URL}/api/connect/google`}>
