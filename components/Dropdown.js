...
export default function Dropdown({allCategories}){
    ...
    const {catRecipes ,setCatRecipes, setIsRecipeCat, setCatVal, catVal, isRecipeCat} = useContext(AuthContext)
    const handleRecCat = (e,index,value)=>{
      e.preventDefault();
      console.log(e.target.value)
      if(catVal === "" || catVal !== e.target.value)setCatVal(e.target.value)
      console.log(catVal)
          ...
          <FormControl className='rounded-md' size='small' fullWidth>
            ...
            >
              ...
              {allCategories && allCategories.map(cat=>(
                <MenuItem value={cat.attributes.name}>{cat.attributes.name}</MenuItem>
              ))}
            </Select>
          </FormControl>
        </Box>
