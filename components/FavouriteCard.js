    ...
    <>
      {isLoaded ? <div>
        <a href="#" className="flex flex-col items-center bg-white rounded-lg border md:flex-row md:w-xl  dark:border-gray-700 dark:bg-gray-800 mb-5">
          <img className="object-cover w-full h-96 rounded-t-lg md:h-48 md:w-48 md:rounded-none md:rounded-l-lg" src={`http://localhost:1337${recipe.attributes && recipe.attributes.post_img.data.attributes.url}`} alt="" />
          <div className=" flex flex-col justify-between px-4 leading-normal">
            ...
            <div className="flex justify-between items-center">
              <p className='dark:text-white text-xl'>Recipe by: <b>{recipe.attributes.creator.data.attributes.username}</b></p>
              <Link
