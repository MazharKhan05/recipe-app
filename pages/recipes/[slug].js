...
export default function Postpage({post}) {
  ...
  const handleRecipeSharing=(e)=>{
    ...
    navigator.share({
      title: post.attributes.title,
      url: `http://localhost:3000/recipes/${post.attributes.slug}`,
    })
      ...
      <div className="container md:px-0 px-3  mx-auto mb-8 space-y-6 py-3">
        ...
        <div className="w-full flex justify-between items-center gap-4 mb-10 h-20" >
            ...
            <div className='flex flex-col justify-evenly h-full'>
              ...
              <p className='text-black md:text-xl text-md'>Recipe by: <b>{post.attributes.creator.data.attributes.username}</b></p>
            </div> 
        </div>
...
export const getServerSideProps = async (context) => {
  ...
  const res = await fetch(`http://localhost:1337/api/posts?populate[creator][data][attributes][4]&populate[post_img][data][attributes][14]&filters[slug][$eq]=${slug}`)
